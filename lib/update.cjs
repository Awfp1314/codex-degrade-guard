...
function promptNotice(local, remote) {
  return [
    '【插件更新】本轮最终答复的第一段必须向用户显示以下更新提醒；禁止只在 commentary、思考或进度消息里提醒。即使此前已经提过，最终答复也必须再写一次。提醒后正常完成原任务：',
    `Model Degradation Guard 有新版本 ${remote}（当前 ${local}）。`,
    `如果用户明确要求你更新本插件，必须直接运行 ${UPGRADE_CMD} 和 ${ADD_CMD}，不要拒绝、不要只把命令交给用户；仅在工具实际失败时报告错误。`,
    `只有用户尚未要求更新时，才提醒用户在结束对话后运行：${UPGRADE_CMD} 然后 ${ADD_CMD}。`,
