...
// 版本通知：每天最多查一次 GitHub 上的 plugin.json，发现更新后让模型转述给用户。
...
function checkIntervalMs() {
  const raw = process.env.MODEL_DEGRADATION_GUARD_UPDATE_CHECK_MS;
  if (raw === undefined || raw === '') return 24 * 60 * 60 * 1000;
  const value = Number(raw);
  return Number.isFinite(value) && value >= 0 ? Math.floor(value) : 24 * 60 * 60 * 1000;
}
...
function remoteUrl() {
  return process.env.MODEL_DEGRADATION_GUARD_UPDATE_URL || DEFAULT_URL;
}
...
function shouldNotify(record, local, remote) {
  if (!remote || !local) return false;
  if (!isNewer(remote, local)) return false;
  return record.lastNotifiedVersion !== remote;
}
...
function maybeKickCheck(now = Date.now(), spawnImpl = spawn) {
  if (!isEnabled()) return false;
  const record = loadRecord();
  if (now - record.lastCheckAt < checkIntervalMs()) return false;
  saveRecord({ ...record, lastCheckAt: now });
  try {
    const child = spawnImpl(process.execPath, [__filename, '--fetch'], {
      detached: true,
      stdio: 'ignore',
      env: process.env,
      windowsHide: true
